...
class AutoClick():
    ...
    def recognize(self, objs):
        ...
        imsrc = ac.imread('../static/src.jpg')
    ...
    def mouse_click(self, x, y, times=0.5):
        ...
        time.sleep(times)
        self.__scroll_down(self.__clickhandle, int(x / scale), int(y))
        # long_position = win32api.MAKELONG(int(x), int(y))
        # win32api.PostMessage(self.__handle, win32con.WM_LBUTTONDOWN, win32con.MK_LBUTTON, long_position)
        # time.sleep(1)
        # win32api.PostMessage(self.__handle, win32con.WM_LBUTTONUP, win32con.MK_LBUTTON, long_position)
    ...
    def mouse_click_image(self, name: str, times=0.5):
        ...
        try:
            ...
            if result is None or result['confidence'] < 0.9:
                ...
                self.mouse_click(result['result'][0], result['result'][1])
...
if __name__ == '__main__':
    ...
    click = AutoClick()
    click.get_winds("上网导航 - 轻快上网 从这里开始")
    click.get_src()
    click.mouse_click_image('./static/ser.png')
